"""
    EfficientNetB0 Evaluation (Test Set)
...
def main():
    ...
    # Paths assumed relative to repo root, efficientnetb0_test.py expected to run from models/classification/
    root_dir = Path(__file__).resolve().parents[2]
    input_csv = os.path.join(root_dir, "data/processed/plastic_standardized_split_data.csv")
    checkpoint_path = os.path.join(Path(__file__).resolve().parent, "plastic_best_efficientnetb0.pt")
    ...
    ''' MODEL DEFINITION '''
    model = models.efficientnet_b0(weights=None) # Weights loaded from checkpoint file
    model.classifier[1] = nn.Linear(model.classifier[1].in_features, 10) # Replace last layer with 10 classes
    ...
    ''' SAVE RESULTS '''
    results_path = Path(__file__).resolve().parent / "plastic_efficientnetb0_test_results.txt"
